...
# File description
kegg_pathway,Annotation relationships between genes/miRNA and KEGG pathways，include kegg_anano.txt and kegg_hsa.gmt file.

brca_data,  breast cancer data.     
blca_data,  bladder cancer data.  
lihc_data,  liver cance data.  
aml_data,Acute Myelocytic Leukemia data
wt_data,nephroblastoma cancer data.

mulcmp_blca.ipynb, blca_data run code.  
mulcmp_lihc.ipynb, lihc_data run code.   
mulcmp_brca.ipynb, brca_data run code.  
multcpm-aml.ipynb, aml_data run code.
multcpm-wt.ipynb, aml_data run code.
